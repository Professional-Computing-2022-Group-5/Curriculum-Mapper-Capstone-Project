...
class AMPLADataset:
    ...
    """An AMPLA Dataset, which stores the data from the AMPLA csv.

    Attributes:
        ampla_events (List): A list of rows from the AMPLA csv.
        floc_to_ampla_events (Dict): A mapping of FLOC -> [ ampla events for that FLOC ]
    """

    def __init__(self, ampla_data_file):
        self.ampla_events = self.read_dataset(ampla_data_file)
        # print(self.ampla_events)
        self.floc_to_ampla_events = self.build_floc_mapping()
        # print(self.floc_to_ampla_events)

    def read_dataset(self, ampla_data_file):
        ampla_events = []
        with open(ampla_data_file, "r") as f:
            reader = csv.DictReader(f)
            for row in reader:
                ampla_events.append({k: row[k] for k in row})
        return ampla_events

    def build_floc_mapping(self):
        """Builds a mapping of FLOC -> [ ampla events for that FLOC].

        Returns:
            Dict: A mapping as above.
        """
        floc_to_ampla_events = {}
        for ampla_event in self.ampla_events:
            floc = ampla_event["EQUIPMENT_ID_CAUSE_LOCATION"]
            if floc not in floc_to_ampla_events:
                floc_to_ampla_events[floc] = []
            floc_to_ampla_events[floc].append(ampla_event)
        return floc_to_ampla_events
    ...
    def read_dataset(
        ...
        with jsonlines.open(data_file, "r") as f:
            ...
            for line in f:
                ...
                # Ignore non-amsul rows
                # if not csv_row['Functional_Location_Sort_Field'].startswith("AMSUL"):
                #   continue
...
class MaintenanceGraphBuilder:
    ...
    Attributes:
        ...
        node_normaliser (LexicalNormaliser): A LexicalNormaliser whose purpose is to clean the nodes, resolving issues such as making 'leaks', 'leaking' etc resolve to a single 'leak' node.
            ampla_dataset (AMPLADataset): An AMPLADataset.
    """
    ...
    def __init__(
        ...
        self.ampla_dataset = ampla_dataset
        self.hierarchy = hierarchy
        self.graph = Graph(password=env("GRAPH_PASSWORD"))
        self.node_normaliser = node_normaliser
        self.defined_fields = defined_fields

    def add_ampla_events(
        self, document, floc_node, relationships, ampla_id_set
    ):  # Incorporate AMPLA Data

        floc = document.structured_fields["Work_Order_Functional_Location"]

        # Check whether the FLOC of this document is present in the floc_to_ampla_events mapping, i.e.
        # the FLOC appears in the AMPLA dataset.
        # If it does, create an AMPLA Event node for each AMPLA Event with the same FLOC.
        if floc in self.ampla_dataset.floc_to_ampla_events:
            ampla_events = self.ampla_dataset.floc_to_ampla_events[floc]
            print(floc, ampla_events)

            for ampla_event in ampla_events:
                ampla_id = ampla_event["Downtime ID"]
                # Only create a node for this AMPLA Event if it has not already been seen.
                if ampla_id not in ampla_id_set:
                    ampla_id_set.add(ampla_id)
                    ...
                    ampla_node = Node(
                        "AMPLA_Event",
                        name=ampla_event["EXPLANATION"],
                        floc_desc=floc_desc,
                        effective_duration=ampla_event["EFF_DURATION"],
                        lost_feed=ampla_event["Nickel Tonnes"],
                    )

                    self.tx.create(ampla_node)

                    # Add a relationship between the Item and AMPLA_Event if it does not already exist.
                    if ampla_node not in relationships:
                        relationships[ampla_node] = {}
                    if floc_node not in relationships[ampla_node]:
                        relationships[ampla_node][floc_node] = {
                            "HAS_AMPLA_EVENT": 1
                        }
    ...
    def build_graph(self):
        ...
        for doc_idx, document in enumerate(self.dataset.documents):
            ...
            if self.ampla_dataset:
                self.add_ampla_events(
                    document, floc_node, relationships, ampla_id_set
                )

                # print("XXXXXXXXXXXXXXXXXXXXXX", relationships[item_node][ampla_node])
                # exit()
...
def main(
    ...
    if ampla_data_file:
        ampla_dataset = AMPLADataset(ampla_data_file)
    else:
        ampla_dataset = None

    node_normaliser = LexicalNormaliser()
    graph_builder = MaintenanceGraphBuilder(
        dataset, hierarchy, node_normaliser, defined_fields, ampla_dataset
    )
...
def build_triples(
    ...
    main(
        ...
        ampla_data_file=ampla_data_filename,
    )
