...
const queryDisplay = ({ data, query }) => {
  ...
  const NodeInputs = () => {
    ...
    // var inputs = nodeTypes.chosenType;
    // console.log(inputs);
    const [nodeDetails, setNodeDetails] = useState({"type": itemType});
  ...
  const NodeCreate = () => {
    ...
    useEffect(() => {
    httpClient.get("//localhost:5000/get_label", {}).then(res => setNodeTypes(res.data));
    }, []);
    ...
    return (
      <div>
        ...
        <Button variant="uwa" onClick={(e) => getInputs(nodeTypes[selectedType], "NODE", selectedType)}>
          Submit
        </Button>
      </div>
    );
  ...
  const LinkInputs = () => {
    const [linkDetails, setLinkDetails] = useState({ 'label': itemType });
    var inputs = ["X", "1", "2", "3"];
  ...
  const LinkCreate = () => {
    ...
    useEffect(() => {
    httpClient.get("//localhost:5000/get_relationship", {}).then(res => setLinkTypes(res.data));
  }, []);
    ...
    return (
      <div>
        ...
        <Button variant="uwa" onClick={(e) => getInputs({}, "LINK", selectedType)}>
          Submit
        </Button>
      </div>
    );
  ...
  useEffect(() => {
    httpClient.post("//localhost:5000/csv",{"query": query}).then(res => setDataInCSV(res.data));
  }, []);
